...
if [[ "$JITTER_MAX_BP" -gt 0 ]]; then
    RUN_SUBDIR="${INPUT_MODE}_classification_${LEFT_BP}_${RIGHT_BP}_jitter${JITTER_MAX_BP}_MLP${HEAD_NUM_LAYERS}"
else
    RUN_SUBDIR="${INPUT_MODE}_classification_${LEFT_BP}_${RIGHT_BP}_MLP${HEAD_NUM_LAYERS}"
fi
...
python $HOME/entexBERT-2/src/entexbert2/scripts/plot_attribution_profiles.py \
  --checkpoint_dir $EXPERIMENT_DIR/output \
  --examples_csv   $EXPERIMENT_DIR/analysis/representative_examples_all.csv \
  --output_dir     $EXPERIMENT_DIR/analysis/attribution \
