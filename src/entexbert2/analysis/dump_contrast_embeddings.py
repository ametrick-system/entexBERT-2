...
Also saved: the model logit ell (= a*s + b = logit P(ASB)) and the binary AS label. If the input
CSV carries total_reads / chr / anchor (build_asb_ism_windows.py emits them), those are passed
through so the head npz gets the SAME leak-free + coverage-matched treatment as the trunk analysis.

Input is a Stage-2 build CSV (columns: sequence1, sequence2, label[, depth]) OR an ASB-windows CSV
(sequence1, sequence2, as_label, total_reads, chr, anchor). Use a held-out / leak-free set.
...
Run ON THE CLUSTER (needs transformers + the trained checkpoint + a GPU is nice but CPU works).
  ...
  python dump_contrast_embeddings.py \
    --checkpoint_dir $ASB/experiments/ctcf/stage2_sft/runs/clf_s20_seed20 \
    --data_csv       $WORK/ctcf_asb_ism_windows_entex.csv --label_col as_label \
    --out            $WORK/head_contrast_ctcf_entex.npz \
...
def main():
    ...
    # carry coverage + coords through so the head npz gets the SAME leak-free + coverage-matched
    # treatment as the trunk ref-embedding analysis (build_asb_ism_windows.py emits these columns).
    def _meta(name, default, cast):
        if name in cols:
            return np.array([cast(r.get(name) or default) for r in rows])
        return np.array([default] * len(rows))
    total_reads = _meta("total_reads", 0.0, float).astype(np.float32)
    chrom = _meta("chr", "?", str)
    anchor = _meta("anchor", 0, lambda v: int(float(v))).astype(np.int64)
