...
class BertUnpadSelfAttention(nn.Module):
    ...
    # NEW: added argument output_attentions and return type to optionally include it in Union (was previously just torch.Tensor)
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor,
        ...
        # NEW: if output_attentions=True, force the PyTorch path because FlashAttention does not materialize the full attention probability tensor!
        if output_attentions or self.p_dropout or flash_attn_qkvpacked_func is None:
...
class BertUnpadAttention(nn.Module):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            self_output = self_outputs
            attention_probs = None
        ################################################

        # NEW LOGIC: don't return immediately (instead save in attention_output)
        if subset_idx is not None:
            attention_output = self.output(index_first_axis(self_output, subset_idx),
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = False, # NEW
        output_hidden_states: Optional[bool] = False, # NEW
    ) -> Tuple[
        List[torch.Tensor],
        Optional[Tuple[torch.Tensor, ...]], # NEW
        Optional[Tuple[torch.Tensor, ...]], # NEW
    ]:

        # NEW #####################################################################
        if subset_mask is not None and (output_attentions or output_hidden_states):
            raise NotImplementedError(
                "Attention/hidden-state extraction is currently implemented for "
                "full-sequence inference only, not masked-token subset inference."
            )
        ############################################################################
        ...
        all_encoder_layers = [] # NEW (moved up in code to before ALiBi)
        all_hidden_states = () if output_hidden_states else None # NEW
        all_self_attentions = () if output_attentions else None # NEW
        ...
        if subset_mask is None:
            for layer_module in self.layer:
                ...
                # NEW
                if output_all_encoded_layers or output_hidden_states:
                    padded_hidden_states = pad_input(
                        hidden_states,
                        indices,
                        batch,
                        seqlen,
            ...
            #     hidden_states[ntokens_unpad,hidden] -> hidden_states[ntokens,hidden]
                ...
                if output_all_encoded_layers:
                    all_encoder_layers.append(padded_hidden_states) # (from before)

                if output_hidden_states:
                    all_hidden_states = all_hidden_states + (padded_hidden_states,)
                ###################################################################
        ...
        return all_encoder_layers, all_hidden_states, all_self_attentions # NEW (all_hidden_states, all_self_attentions)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None, # NEW
        output_hidden_states: Optional[bool] = None, # NEW
        return_dict: Optional[bool] = None, # NEW
        **kwargs
    ) -> Union[Tuple[Union[List[torch.Tensor], torch.Tensor], Optional[torch.Tensor]], BaseModelOutputWithPooling]: # NEW (BaseModelOutputWithPooling)
        ...
        encoder_outputs, all_hidden_states, all_self_attentions = self.encoder( # NEW (all_hidden_states, all_self_attentions)
            embedding_output,
            attention_mask,
        ...
        # self.pooler is not None logic from original code
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        return MaskedLMOutput(
            ...
            hidden_states=outputs.hidden_states, # NEW (changed from hidden_states=outputs[0])
            attentions=outputs.attentions, # NEW (changed from attentions=None)
        )
